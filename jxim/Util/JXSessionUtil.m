...
static NSString *const JXRecentSessionAtMark  = @"JXRecentSessionAtMark";
static NSString *const JXRecentSessionTopMark = @"JXRecentSessionTopMark";

@implementation JXSessionUtil
...
+(NSString*)showTime:(NSTimeInterval) msglastTime showDetail:(BOOL)showDetail
{
    ...
    result = [JXSessionUtil getPeriodOfTime:hour withMinute:msgDateComponents.minute];
    if (hour > 12)
    {
        hour = hour - 12;
    }
    ...
    {
        NSString *weekDay = [JXSessionUtil weekdayStr:msgDateComponents.weekday];
...
+ (NSDictionary *)dictByJsonString:(NSString *)jsonString
{
    ...
    return [JXSessionUtil dictByJsonData:data];
}
...
+ (NSString *)tipOnMessageRevoked:(NIMRevokeMessageNotification *)notification
{
    ...
    return [NSString stringWithFormat:@"%@%@", tip, @"撤回了一条消息"];
}
...
+ (BOOL)canMessageBeForwarded:(NIMMessage *)message
{
    ...
    {
        id<JXCustomAttachmentInfo> attach = (id<JXCustomAttachmentInfo>)[(NIMCustomObject *)message.messageObject attachment];
...
+ (void)addRecentSessionMark:(NIMSession *)session type:(JXRecentSessionMarkType)type
{
    ...
    if (recent)
    {
        ...
        NSString *key = [JXSessionUtil keyForMarkType:type];
...
+ (NSString *)keyForMarkType:(JXRecentSessionMarkType)type
{
    static NSDictionary *keys;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        keys = @{
                 @(JXRecentSessionMarkTypeAt)  : JXRecentSessionAtMark,
                 @(JXRecentSessionMarkTypeTop) : JXRecentSessionTopMark
                 };
    });
...
{
    NSString *state = @"";
    if (![JXSubscribeManager sharedManager] || [[NIMSDK sharedSDK].loginManager.currentAccount isEqualToString:userId])
    {
        //没有开启订阅服务或是自己  不显示在线状态
        return state;
    }
    
    NSDictionary *dict = [[JXSubscribeManager sharedManager] eventsForType:NIMSubscribeSystemEventTypeOnline];
    ...
    if ([info isKindOfClass:[NIMSubscribeOnlineInfo class]] && info.senderClientTypes.count)
    {
        ...
        switch (event.value) {
            case JXCustomStateValueOnlineExt:
            ...
            default:
            {
                ...
                state = [NSString stringWithFormat:@"%@ %@", clientName, @"在线"];
                break;
            }
        }
    }
...
{
    NSString *clientName = [self resolveOnlineClientName:client];
    NSString *state = [NSString stringWithFormat:@"%@ %@",clientName,@"在线"];
    NSDictionary *dict = [ext nimkit_jsonDict];
    if (dict) {
        
        NSString *netState =  [[JXDevice currentDevice] networkStatus:[dict jsonInteger:JXSubscribeNetState]];
        JXOnlineState onlineState =  [dict jsonInteger:JXSubscribeOnlineState];
        switch (onlineState) {
            case JXOnlineStateNormal:
            {
                ...
                {
                    //桌面端不显示网络状态，只显示端
                    return [NSString stringWithFormat:@"%@ %@",clientName,@"在线"];
                }
                else
                {
                    ...
                    if (detail)
                    {
                        return [NSString stringWithFormat:@"%@ - %@ %@",clientName,netState, @"在线"];
                    }
                    else
                    {
                        return [NSString stringWithFormat:@"%@ %@",netState,@"在线"];
                    }
                }
            }
            case JXOnlineStateBusy:
                return @"忙碌";
            case JXOnlineStateLeave:
                return @"离开";
            default:
                break;
        }
