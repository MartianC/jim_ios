...
@interface JXCellLayoutConfig ()
@property (nonatomic,strong)    NSArray    *types;
@property (nonatomic,strong)    JXSessionCustomContentConfig  *sessionCustomconfig;
@end

@implementation JXCellLayoutConfig
...
- (instancetype)init
{
    ...
    {
        _types =  @[
                   @"JXJanKenPonAttachment",
                   @"JXSnapchatAttachment",
                   @"JXChartletAttachment",
                   @"JXRedPacketAttachment",
                   @"JXRedPacketTipAttachment",
                   @"JXMultiRetweetAttachment"
                   ];
        _sessionCustomconfig = [[JXSessionCustomContentConfig alloc] init];
    }
    return self;
}
...
{
    if ([model.message.localExt.allKeys containsObject:JXMessageRefusedTag])
    {
        return [[model.message.localExt objectForKey:JXMessageRefusedTag] boolValue];
    }
...
- (BOOL)isRedpacketTip:(NIMMessage *)message
{
    if (message.messageType == NIMMessageTypeCustom) {
        NIMCustomObject *object = message.messageObject;
        if ([object.attachment isKindOfClass:[JXRedPacketTipAttachment class]]) {
            return YES;
        }
    }
    return NO;
