...
@interface JXMultiRetweetAttachment ()
...
@implementation JXMultiRetweetAttachment
...
- (void)setUrl:(NSString *)url {
    ...
    if (url && !_filePath) {
        ...
        _filePath = [JXFileLocationHelper filepathForMergeForwardFile:fileName];
    }
}

- (void)setAbstracts:(NSMutableArray<JXMessageAbstract *> *)abstracts {
    _abstracts = abstracts;
    NSMutableArray *abstractDics = [NSMutableArray array];
    for (JXMessageAbstract *obj in abstracts) {
    ...
    if (!messageAbstract) {
        ...
        for (id obj in messageAbstract) {
            if ([obj isKindOfClass:[NSDictionary class]]) {
                JXMessageAbstract *abstract = [JXMessageAbstract abstractWithDictionary:obj];
                if (abstract) {
...
- (BOOL)attachmentNeedsDownload {
    ...
    if (_filePath.length == 0) {
        _filePath = [JXFileLocationHelper filepathForMergeForwardFile:_url.lastPathComponent];
    }
...
- (NSString *)cellContent:(NIMMessage *)message {
    return @"JXSessionMultiRetweetContentView";
}
...
- (CGSize)contentSize:(NIMMessage *)message cellWidth:(CGFloat)width {
    ...
    for (JXMessageAbstract *abs in _abstracts) {
...
#pragma mark - JXMessageAbstract
#define JX_Message_Abstract_Max_Len (32)

@implementation JXMessageAbstract
...
+ (instancetype)abstractWithMessage:(NIMMessage *)message {
    ...
    JXMessageAbstract *ret = [[JXMessageAbstract alloc] init];
...
- (NSString *)abstract:(NIMMessage *)message {
    NSString *msg = [JXMessageUtil messageContent:message];
    NSMutableString *ret = [NSMutableString string];
    if (msg.length > JX_Message_Abstract_Max_Len) {
        NSArray *tokens = [[NIMInputEmoticonParser currentParser] tokens:msg];
        for (NIMInputTextToken *token in tokens) { //防止emoji表情被截断
            if (ret.length > JX_Message_Abstract_Max_Len) {
                break;
            }
