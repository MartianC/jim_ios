...
static dispatch_queue_t JXMessageSerializationQueue()
{
...
@interface JXMessageSerialization ()
...
@implementation JXMessageSerialization
...
- (void)encode:(NSArray <NIMMessage *>*)messages
    ...
    dispatch_async(JXMessageSerializationQueue(), ^{
        ...
        if (!headerRowData) {
            dispatch_async(dispatch_get_main_queue(), ^{
                error = [NSError errorWithDomain:@"JX.message.serialization" code:1000 userInfo:nil];
                if (completion) {
        ...
        if (messageRowData.count == 0) {
            dispatch_async(dispatch_get_main_queue(), ^{
                error = [NSError errorWithDomain:@"JX.message.serialization" code:1001 userInfo:nil];
                if (completion) {
                    completion(error, nil);
                }
            });
        ...
        if (file == nil) {
            error = [NSError errorWithDomain:@"JX.message.serialization" code:1002 userInfo:nil];
            dispatch_async(dispatch_get_main_queue(), ^{
                if (completion) {
                    completion(error, nil);
                }
        ...
        //md5
        NSString *md5 = [JXFileUtil fileMD5:file];
        if (!md5) {
            //delete
            [[NSFileManager defaultManager] removeItemAtPath:file error:nil];
            
            //error
            error = [NSError errorWithDomain:@"JX.message.serialization" code:1003 userInfo:nil];
            dispatch_async(dispatch_get_main_queue(), ^{
                if (completion) {
                    completion(error, nil);
                }
        ...
        //callback
        JXMessageSerializationInfo *info = [[JXMessageSerializationInfo alloc] init];
        info.filePath = file;
        info.md5 = md5;
        info.encrypted = YES;
...
- (NSString *)targetFile {
    ...
    return [JXFileLocationHelper filepathForMergeForwardFile:fileName];
}

- (NSData *)fileHeaderWithCount:(NSInteger)count {
    JXMigrateHeader *header = [JXMigrateHeader initWithDefaultConfig];
...
- (NSMutableArray <NIMMessage *>*)readRowDatasFromFile:(NSString *)file
    ...
    for (int i = 0; i < subStrings.count; i++) {
        ...
        if (i == 0) {
            ...
            header = [JXMigrateHeader initWithRawContent:headerData];
            if (header == nil) {
                break;
            }
        } else {
...
@implementation JXMessageSerializationInfo
