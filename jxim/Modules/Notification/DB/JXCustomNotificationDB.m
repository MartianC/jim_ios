...
@interface JXCustomNotificationDB ()
@property (nonatomic,strong)  FMDatabase *db;
@end


@implementation JXCustomNotificationDB
...
- (NSArray *)fetchNotifications:(JXCustomNotificationObject *)notification
    ...
    io_sync_safe(^{
        ...
        while ([rs next])
        {
            JXCustomNotificationObject *notification = [[JXCustomNotificationObject alloc] init];
...
- (void)openDatabase
{
    NSString *filepath = [[JXFileLocationHelper userDirectory] stringByAppendingString:@"notification.db"];
...
static const void * const JXDispatchIOSpecificKey = &JXDispatchIOSpecificKey;
dispatch_queue_t JXDispatchIOQueue()
{
    static dispatch_queue_t queue;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        queue = dispatch_queue_create("nim.demo.io.queue", 0);
        dispatch_queue_set_specific(queue, JXDispatchIOSpecificKey, (void *)JXDispatchIOSpecificKey, NULL);
    });
    return queue;
}
...
{
    if (dispatch_get_specific(JXDispatchIOSpecificKey))
    {
        block();
    }
    else
    {
        dispatch_sync(JXDispatchIOQueue(), ^() {
            block();
        });
    }
}
...
void io_async(dispatch_block block){
    dispatch_async(JXDispatchIOQueue(), ^() {
        block();
    });
}
