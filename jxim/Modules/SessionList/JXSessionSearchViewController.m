//
//  JXSessionSearchViewController.m
//  jxim
//
...
@interface JXSessionSearchViewController() <UISearchResultsUpdating, UISearchControllerDelegate>
...
@implementation JXSessionSearchViewController
...
- (void)viewDidLoad {
    ...
    if (!IOS11) {
        CGFloat insetTop = self.searchVC.searchBar.height + STATUS_BAR_SIZE.height;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (tableView == self.searchResultVC.tableView) {
        ...
        JXSessionViewController *vc = [[JXSessionViewController alloc] initWithSession:recent.session];
...
- (NSString *)messageContent:(NIMMessage*)lastMessage{
    NSString *text = [JXMessageUtil messageContent:lastMessage];
...
- (UITableViewController *)searchResultVC {
    if (!_searchResultVC) {
        ...
        CGFloat resultInsetTop = SEARCHBAR_HEIGHT;
...
- (UISearchController *)searchVC {
    if (!_searchVC) {
        ...
        [_searchVC.searchBar setFrame:CGRectMake(0, 0, SCREEN_WIDTH, SEARCHBAR_HEIGHT)];
    }
    return _searchVC;
}
