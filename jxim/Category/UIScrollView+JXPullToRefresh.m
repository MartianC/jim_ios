//
// UIScrollView+JXPullToRefresh.m
//

#import <QuartzCore/QuartzCore.h>
#import "UIScrollView+JXPullToRefresh.h"

#define fequal(a,b) (fabs((a) - (b)) < FLT_EPSILON)
#define fequalzero(a) (fabs(a) < FLT_EPSILON)

static CGFloat const JXPullToRefreshViewHeight = 60;

@interface JXPullToRefreshArrow : UIView
...
@interface JXPullToRefreshView ()

@property (nonatomic, copy) void (^pullToRefreshActionHandler)(void);

@property (nonatomic, strong) JXPullToRefreshArrow *arrow;
@property (nonatomic, strong) UIActivityIndicatorView *activityIndicatorView;
@property (nonatomic, strong, readwrite) UILabel *titleLabel;
@property (nonatomic, strong, readwrite) UILabel *subtitleLabel;
@property (nonatomic, readwrite) JXPullToRefreshState state;
@property (nonatomic, readwrite) JXPullToRefreshPosition position;
...
#pragma mark - UIScrollView (JXPullToRefresh)
#import <objc/runtime.h>

static char UIScrollViewPullToRefreshView;

@implementation UIScrollView (JXPullToRefresh)
...
- (void)addPullToRefreshWithActionHandler:(void (^)(void))actionHandler position:(JXPullToRefreshPosition)position {
    ...
    if(!self.pullToRefreshView) {
        CGFloat yOrigin;
        switch (position) {
            case JXPullToRefreshPositionTop:
                yOrigin = -JXPullToRefreshViewHeight;
                break;
            case JXPullToRefreshPositionBottom:
                yOrigin = self.contentSize.height;
                break;
            default:
                return;
        }
        JXPullToRefreshView *view = [[JXPullToRefreshView alloc] initWithFrame:CGRectMake(0, yOrigin, self.bounds.size.width, JXPullToRefreshViewHeight)];
...
- (void)addPullToRefreshWithActionHandler:(void (^)(void))actionHandler {
    [self addPullToRefreshWithActionHandler:actionHandler position:JXPullToRefreshPositionTop];
}

- (void)triggerPullToRefresh {
    self.pullToRefreshView.state = JXPullToRefreshStateTriggered;
    [self.pullToRefreshView startAnimating];
}

- (void)setPullToRefreshView:(JXPullToRefreshView *)pullToRefreshView {
...
- (JXPullToRefreshView *)pullToRefreshView {
...
- (void)setShowsPullToRefresh:(BOOL)showsPullToRefresh {
    ...
    if(!showsPullToRefresh) {
        ...
        if (!self.pullToRefreshView.isObserving) {
            ...
            self.pullToRefreshView.frame = CGRectMake(0, yOrigin, self.bounds.size.width, JXPullToRefreshViewHeight);
        }
    }
}
...
#pragma mark - JXPullToRefresh
@implementation JXPullToRefreshView
...
- (id)initWithFrame:(CGRect)frame {
    if(self = [super initWithFrame:frame]) {
        ...
        self.state = JXPullToRefreshStateStopped;
...
- (void)layoutSubviews {
    ...
    if(hasCustomView) {
        ...
        switch (self.state) {
            case JXPullToRefreshStateAll:
            case JXPullToRefreshStateStopped:
                self.arrow.alpha = 1;
            ...
            case JXPullToRefreshStateTriggered:
                switch (self.position) {
                    case JXPullToRefreshPositionTop:
                        [self rotateArrow:(float)M_PI hide:NO];
                        break;
            ...
            case JXPullToRefreshStateLoading:
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    if([keyPath isEqualToString:@"contentOffset"])
        ...
        self.frame = CGRectMake(0, yOrigin, self.bounds.size.width, JXPullToRefreshViewHeight);
    }
...
- (void)scrollViewDidScroll:(CGPoint)contentOffset {
    if(self.state != JXPullToRefreshStateLoading) {
        ...
        if(!self.scrollView.isDragging && self.state == JXPullToRefreshStateTriggered)
            self.state = JXPullToRefreshStateLoading;
        else if(contentOffset.y < scrollOffsetThreshold && self.scrollView.isDragging && self.state == JXPullToRefreshStateStopped && self.position == JXPullToRefreshPositionTop)
            self.state = JXPullToRefreshStateTriggered;
        else if(contentOffset.y >= scrollOffsetThreshold && self.state != JXPullToRefreshStateStopped && self.position == JXPullToRefreshPositionTop)
            self.state = JXPullToRefreshStateStopped;
        else if(contentOffset.y > scrollOffsetThreshold && self.scrollView.isDragging && self.state == JXPullToRefreshStateStopped && self.position == JXPullToRefreshPositionBottom)
            self.state = JXPullToRefreshStateTriggered;
        else if(contentOffset.y <= scrollOffsetThreshold && self.state != JXPullToRefreshStateStopped && self.position == JXPullToRefreshPositionBottom)
            self.state = JXPullToRefreshStateStopped;
    } else {
        CGFloat offset;
...
- (JXPullToRefreshArrow *)arrow {
    if(!_arrow) {
        _arrow = [[JXPullToRefreshArrow alloc]initWithFrame:CGRectMake(0, self.bounds.size.height-54, 22, 48)];
...
- (void)setTitle:(NSString *)title forState:(JXPullToRefreshState)state {
    ...
    if(state == JXPullToRefreshStateAll)
...
- (void)setState:(JXPullToRefreshState)newState {
    
    if(_state == newState)
        return;
    
    JXPullToRefreshState previousState = _state;
    _state = newState;
    ...
    switch (newState) {
        ...
        case JXPullToRefreshStateLoading:
            ...
            if(previousState == JXPullToRefreshStateTriggered && pullToRefreshActionHandler)
...
#pragma mark - JXPullToRefreshArrow

@implementation JXPullToRefreshArrow
