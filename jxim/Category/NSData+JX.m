...
void _JX_RC4(const char *key, int keylen, char* output, const char* input, int len)
{
    // 设置密钥
    char key_[256];
    memset(key_, 0, 256);
...
- (NSData *)rc4EncryptWithKey:(NSString *)key {
    ...
    _JX_RC4(pwd, (int)key.length, outdata, self.bytes, (int)self.length);
